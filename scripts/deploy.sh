...
ssh $EC2_INSTANCE_USERNAME@$EC2_INSTANCE_IP_ADDRESS "\
  ...
  echo $SUDO | sudo -S systemctl stop docker && \
  echo $SUDO | sudo -S rm -rf /var/lib/docker/* && \
  echo $SUDO | sudo -S systemctl start docker && \
  ...
  docker run -e PORT=$PORT -d -v ~/.aws:/root/.aws -p $PORT:$PORT --name $CIRCLE_PROJECT_REPONAME $ECR/$CIRCLE_PROJECT_REPONAME:latest"
